...
Apify.main(async () => {
    ...
    const crawler = new Apify.PuppeteerCrawler({
        ...
        handlePageFunction: async ({ request, page, response }) => {
            ...
            if (await page.$('.dv-transcript-container') !== null){
                record.dvText = true;
            } else if (await page.$('.dv-toggle_container') !== null){
                record.dvAudio = true;  
            } 
...
<html>
  <head>
    <title>DV Audio/DV Text report for ${baseUrl}</title>
    <style>
        body {
  ...
  <body>
    ...
    for (let link of results[0]['links']) {
        if (link.url.includes('https://www.tvo.org/video')){
            console.log(link);
            ...
            //} else 
            if ((link.errorMessage || !link.httpStatus || link.httpStatus < 200 || link.httpStatus >= 300)) {
                color = 'red';
                description = link.errorMessage ? `Error: ${link.errorMessage}` : 'Invalid HTTP status';
            } else if (link.dvText == false && link.dvAudio == false){
                color = 'red';
                description = 'No DV Audio or DV Text';
            }
            if (link.httpStatus !== 200 || (link.httpStatus == 200 && link.dvText == false && link.dvAudio == false )){
